...
function trelloCalendarApp(config) {
    ...
    app.get(/\/trello\/(.+)/, function(req, res) {
        ...
        req.trello.get('/'+ req.params[0], req.query, function(err, data) {
            ...
            if (req.params[0].indexOf('boards/') == 0) {
                /* Hack: embed the checklists to the cards */
                var getChecklist = function (idChecklist, callback) {
                    req.trello.get('/checklists/' + idChecklist, function (err, data) {
                        if (err) {
                            return callback(err);
                        }
                ...
                var getCardChecklists = function (card, callback) {
                    async.map(card.idChecklists, getChecklist, function (err, data) {
                        if (err) {
                            return callback(err);
                        }
                        card.checklists = data;
                        callback (null, data);
                    })
                }

                async.map(data, getCardChecklists, function (err, result) {
                    if (err) {
                        console.error(err)
                        return sendRerror(res)
                    }
                    res.json(data)
                })
            } else {
